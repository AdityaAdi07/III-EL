...
// Fetch the subject handled by the teacher
$query_subject = "SELECT subject FROM admin WHERE user_id = '$teacher_id'";
$result_subject = mysqli_query($con, $query_subject);

if (mysqli_num_rows($result_subject) == 0) {
    echo "No subject found for the logged-in teacher.";
    exit;
}

$subject = mysqli_fetch_assoc($result_subject)['subject'];

// Fetch existing sessions for the teacher
$query_sessions = "SELECT DISTINCT date, time FROM attendance WHERE teacher_id = '$teacher_id'";
$result_sessions = mysqli_query($con, $query_sessions);

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['existing_session'])) {
        list($date, $time) = explode('|', $_POST['existing_session']);
    } else {
        $date = $_POST['date'];
        $time = $_POST['time'];
    }
  ...
  <div class="topbar">
    ...
    <div class="sidebar"><div><img class="home" src="/Images/HomeIcon.png"></div>
      <div><img class="aboutus" src="/Images/stats-removebg-preview.png"> </div>
        ...
        <h2>Create a New Session or Select an Existing Session</h2>
    <form method="POST">
        <label for="existing_session">Select Existing Session:</label>
        <select id="existing_session" name="existing_session">
            <option value="">--Select a Session--</option>
            <?php while ($session = mysqli_fetch_assoc($result_sessions)) : ?>
                <option value="<?php echo htmlspecialchars($session['date'] . '|' . $session['time']); ?>">
                    <?php echo htmlspecialchars($session['date'] . ' - ' . $session['time']); ?>
                </option>
            <?php endwhile; ?>
        </select>
        <br><br>
        ...
        <br><br>
        <input type="submit" value="Create/Select Session">
    </form>
    </div>