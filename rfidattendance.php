...
// Check if RFID data is posted
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rfid_tag'])) {
    $rfid_tag = mysqli_real_escape_string($con, $_POST['rfid_tag']);

    // Fetch user information based on RFID tag
    $query_user = "SELECT * FROM users WHERE rfid_tag_id = '$rfid_tag'";
    ...
    if (mysqli_num_rows($result_user) > 0) {
        ...
        $query_attendance = "INSERT INTO attendance (user_id, timestamp) VALUES ('$user_id', '$timestamp')";
        if (mysqli_query($con, $query_attendance)) {
            echo "Attendance marked successfully for user: " . $user_row['username'];
        } else {
            echo "Error marking attendance: " . mysqli_error($con);
        }
    } else {
        echo "No user found with this RFID tag.";
    }
}
?>
<!DOCTYPE html>
...
<div class="rfid">
     <!-- Form for RFID tag scanning -->
     <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <input type="text" name="rfid_tag" placeholder="Scan RFID tag..." required>
